...
bool ggml_cuda_gdn_op_is_chunked(const ggml_tensor * dst) {
    ...
    const bool arch_ok = (GGML_CUDA_CC_IS_NVIDIA(cc_dev) && cc_dev >= GGML_CUDA_CC_AMPERE)
                      ...
                      || ((GGML_CUDA_CC_IS_RDNA3(cc_dev) || GGML_CUDA_CC_IS_RDNA4(cc_dev)) && rdna_opt_in);

    // - NVIDIA Ampere+ / CDNA / RDNA3+RDNA4 (opt-in): fp16 tensor cores; not KDA; K == 1 (final state only)
